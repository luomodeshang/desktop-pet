...
| 版本 | 亮点 | 详细说明 |
|------|------|----------|
| **V4.0.0** | MediaPipe FaceMesh面部渲染 | MediaPipe 478点3D面部网格替代Haar Cascade；精确脸型基于实际landmarks；个性化眼型/嘴型/鼻型；层次头发/身体渲染；Live2D表情系统升级；纯CPU运行 |
| **v3.0.18** | 位置修正+食物缓存修复 | 所有动画坐标基于角色实际显示区域(画布底部70%)；食物图片选择缓存BUG修复(不同食物类型会重建缓存)；食物尺寸从16%放大到20%画布宽；挥手肩膀/嘴位置下移至角色实际区域 |
| **v3.0.17** | 全比例动画+真实食物图 | 所有动画(挥手/喂食/跳跃)坐标按画布尺寸比例缩放；跳跃高度随尺寸自适应(100px=12px,300px=36px)；喂食使用真实食物PNG图片；小手尺寸50px时挥手正确右移 |
| **v3.0.16** | 人物缩小+跳跃30%+双手喂食 | 人物缩小至70%更贴合窗口；跳跃高度降至30px；喂食改为左右手从两侧伸出接食物放进嘴；食物图片更明显区分 |
| **v3.0.15** | 喂食手接动作+Size修复 | 食物从天而降，手伸出接住后吃掉(三段动画)；调整大小时不再丢失角色 |
| **v3.0.14** | 跳得更高+食物区分修复 | 跳跃高度从65px增加到100px；修复食物匹配逻辑(Fried Chicken不再错误画成汉堡) |
| **v3.0.13** | 窗体加高+食物区分 | 窗口高度+80px防止跳跃出界；三种食物不同图案(汉堡/鸡腿/面条碗) |
