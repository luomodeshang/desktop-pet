...
echo ========================================
echo.
echo This script downloads the pre-built EXE version
...
curl -L -o "%INSTALL_DIR%\DesktopPet.exe" "%EXE_URL%" 2>nul
...
bitsadmin /transfer DesktopPetDownload /download /priority normal "%EXE_URL%" "%INSTALL_DIR%\DesktopPet.exe" 2>nul
...
echo [ERROR] Failed to download DesktopPet.exe
...
echo oShellLink.TargetPath = "%INSTALL_DIR%\DesktopPet.exe" >> "%TEMP%\shortcut.vbs"
...
echo 2. Or double-click DesktopPet.exe in the folder
echo 3. Or run: %INSTALL_DIR%\run.bat
echo.
echo IMPORTANT: Windows Defender may flag the EXE as suspicious
...
if errorlevel 2 (
    ...
    start "" "%INSTALL_DIR%\DesktopPet.exe"
)
